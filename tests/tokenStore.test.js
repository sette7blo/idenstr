...
test('API tokens are shown once and stored only as hashes', async () => {
  ...
  try {
    process.env.IDENSTR_DB_STORE = join(dir, 'idenstr.db');
    const store = new TokenStore(join(dir, 'tokens.json'));
    const created = await store.createToken('Feedstr link', ['profile:read', 'following:read']);

    assert.equal(created.name, 'Feedstr link');
    assert.match(created.token, /^idstr_[a-zA-Z0-9_-]+$/);
    ...
    assert.equal(await store.verifyToken(created.token, 'admin'), false);
  } finally {
